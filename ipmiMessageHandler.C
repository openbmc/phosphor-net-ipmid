...
IpmiMessageHandler::IpmiMessageHandler(std::shared_ptr<IpmiSockChannelData> i_channel)
...
IpmiMessageHandler::~IpmiMessageHandler() {
  if (iv_inMsg) { delete iv_inMsg ; iv_inMsg = nullptr;}
  if (iv_outMsg) { delete iv_outMsg; iv_outMsg = nullptr;}
}

//Extract payload
void IpmiMessageHandler::receive() {
  std::cout<<">> IpmiMessageHandler::receive"<<std::endl;
  ...
  do {
    ...
    // Read the Session ID
    std::shared_ptr<IpmiSession> l_session = IpmiSessionsManager::getInstance().getSession
                                              (iv_inMsg->getSessionId().get());

    if (l_session == nullptr) {
      //The requested session is not found .. drop the message.
      if (iv_inMsg) {
        std::cerr<<"E> IpmiMessageHandler::receive: No session available !!!"<<std::endl;
        delete iv_inMsg;
        iv_inMsg = nullptr;
        break;
      }
      ...
      if (l_session->getBMCSessionID().get() == 0x00) {
        std::cout<<"I> IpmiMessageHandler::receive: Session ID is zero"<<std::endl;
      }
      ...
      if (l_session->getBMCSessionID().get() == 0x00) {
        if ((l_session->getSessionState().isSessionActive()) &&
           ...
           (strcmp(iv_channel->getRemoteAddress(), l_session->getChannel()->getRemoteAddress())
                                                                                 != 0))) {
          ...
          delete iv_inMsg;
            iv_inMsg = nullptr;
            std::cerr<<"E> IpmiMessageHandler::receive: Session Zero is already active!"<<std::endl;
            break;
          }
        }
    ...
    if (iv_ipmiFrame == nullptr) {
      std::cerr<<"E> IpmiMessageHandler::receive: Unflattening the payload failed !!!"<<std::endl;
      delete iv_inMsg;
      iv_inMsg = nullptr;
    }
    iv_ipmiFrameLength = iv_inMsg->getPayloadLength().get();
...
void IpmiMessageHandler::send() {
  std::cout<<">> IpmiMessageHandler::send"<<std::endl;

  IpmiMessageParser* l_parser = nullptr;

  // Get the message parser type from Incoming Packet
  if(iv_inMsg) {
    l_parser = IpmiMessageParser::getParser(iv_inMsg->getPacket(), iv_inMsg->getPacketLength());
  }
  ...
  if (iv_session &&
    ...
    ipmiLanMsgResponseHeader* l_lanMsgRsHeader = (ipmiLanMsgResponseHeader*) ((void*)iv_ipmiFrame);
    ipmiLanMsgRequestHeader* l_lanMsgRqHeader =  (ipmiLanMsgRequestHeader*) ((void*)iv_ipmiFrame);
    ...
    l_lanMsgRsTrailer.checksum2 = ipmiCrc8bit(&l_lanMsgRsHeader->rsaddr, l_assembledSize -3);
    ...
    iv_outMsg = new IpmiMessage(iv_session->getRCSessionID(), iv_session->getBMCSessionID(),
    ...
    if (iv_channel) {
      ...
      std::cerr<<"E> IpmiMessageHandler::send: iv_channel is NULL !!!"<<std::endl;
    }

    if (l_rc < 0) {
      std::cerr<<"E> IpmiMessageHandler::send: Error in iv_outMsg->Send !!!"<<std::endl;
      l_rc = 0;
    }
...
void IpmiMessageHandler::rawSend(IpmiPayloadTypes i_payloadType) {
  if (iv_responsePayloadSize != 0) {
    std::cout<<">> IpmiMessageHandler::rawSend"<<std::endl;
    ...
    if(l_rc < 0) {
      std::cerr<<"E> IpmiMessageHandler::rawSend: Error in iv_outMsg->Send !!!"<<std::endl;
      l_rc = 0;
    }
...
uint16_t IpmiMessageHandler::getSessionPortNumber(void) {
  ...
  if (iv_session) {
    ...
    if(!getsockname(iv_session->getChannel()->getHandle(),
      ...
      std::cerr<<"E> Error in getsockname: Errno: 0x"<<std::hex<<errno<<std::endl;
    }
  }
...
void IpmiMessageHandler::activateShutdown(void) {
  if (iv_session && iv_session->getBMCSessionID().get() != 0) {
    IpmiSessionsManager::getInstance().stopSession(iv_session->getBMCSessionID().get());
  }
}
...
bool IpmiMessageHandler::wasShutdownActivated(void) {
  ...
  if (iv_session) {
    if (iv_session->getBMCSessionID().get() == 0) {
      l_ret = false;
    } else {
        l_ret = (iv_session->getSessionState().getSessionState()
                != IpmiSessionState::IPMI_SESSION_IS_ACTIVE) ? true : false;
    }
  }
...
uint8_t IpmiMessageHandler::getPayloadType() {
  return iv_inMsg?iv_inMsg->getPayloadType() : IpmiMessage::IPMI_PAYLOAD_TYPE_INVALID;
}
...
uint32_t IpmiMessageHandler::getCommand(void) {
  ...
  if (iv_inMsg) {
    ...
    if (iv_inMsg->getPayloadType() == IpmiMessage::IPMI_PAYLOAD_TYPE_IPMI) {
      l_cmd |= ((reinterpret_cast<ipmiLanMsgRequestHeader*>(iv_ipmiFrame))->netfn)<<8;
...
void IpmiMessageHandler::route(void) {
  std::cout<<">> IpmiMessageHandler::route"<<std::endl;
  if (iv_inMsg == nullptr) {
    std::cerr<<"E> IpmiMessageHandler::route iv_inMsg == NULL !!!"<<std::endl;
  }

  if (iv_inMsg) {
    std::cout<<"I> IpmiMessageHandler::route Execute command"<<std::endl;
    IpmiCommandTable::getInstance().ExecuteCommand(getCommand(),*this);
  }
}
...
uint8_t ipmiProcessDataLen(uint8_t i_ipmiDataLen, IpmiMessageHandler& i_sessCtl) {
  uint8_t l_rc = IPMICC_NORMAL;

  // Validate the Request data length
  if (i_ipmiDataLen != (i_sessCtl.iv_requestPayloadSize)) l_rc = IPMICC_REQ_DATA_LEN_INVALID;

  return l_rc;
}

uint8_t ipmiProcessDataLenRange(uint8_t i_ipmiDataLenLower, uint16_t i_ipmiDataLenUpper,
