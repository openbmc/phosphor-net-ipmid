    ...
    message::Message* inMessage)
{
    ...
    if (inMessage->payloadType == message::PayloadType::IPMI)
    {
        inMessage->payload.erase(inMessage->payload.begin(),
                                 inMessage->payload.begin() + sizeof(message::LanMsgRequestHeader));
        inMessage->payload.erase(inMessage->payload.end() - sizeof(
                                     message::LanMsgTrailer),
    ...
    if (inMessage->payloadType == message::PayloadType::IPMI)
    {
        ...
        outMessage->payload.resize(sizeof(message::LanMsgResponseHeader) + output.size()
                                   +
                                   sizeof(message::LanMsgTrailer));

        auto reqHeader = reinterpret_cast<message::LanMsgRequestHeader*>
                         (inPayload.data());
        auto respHeader = reinterpret_cast<message::LanMsgResponseHeader*>
        ...
        auto assembledSize = sizeof(message::LanMsgResponseHeader);
        ...
        auto trailer = reinterpret_cast<message::LanMsgTrailer*>
...
uint32_t MessageHandler::getCommand(message::Message* message)
{
    ...
    if (message)
    {
        ...
        {
            command |= ((reinterpret_cast<message::LanMsgRequestHeader*>
