                        ...
                        const std::string& service, const std::string& obj)
{
    ...
    {
        lg2::error(
            "Failed in getting session state property: {SERVICE}, {PATH}, {INTF}",
            "SERVICE", service, "PATH", obj, "INTF", session::sessionIntf);
                                     ...
                                     const uint8_t currentSessionPriv)
{
    ...
    {
        lg2::error(
            "Failed to fetch object from dbus, interface: {INTF}, error: {ERROR}",
            "INTF", session::sessionIntf, "ERROR", e);
        return ipmi::ccUnspecifiedError;
    }
