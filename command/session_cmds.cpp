...

{
    ...
    uint8_t i = 0;
    std::vector<std::string> ipStrBytes;
    char remoteIPinByte[4];
    response->completionCode = IPMI_CC_OK;
    ...
    if (reqSessionID != 0)
    {
        ...
        auto str = sessionInfo->channelPtr->getRemoteAddress();
        std::size_t pos = str.rfind(":");
        std::string remoteIPStr = str.substr(pos + 1);
        boost::split(ipStrBytes, remoteIPStr, boost::is_any_of("."));
        for (std::vector<std::string>::iterator it = ipStrBytes.begin();
             it != ipStrBytes.end(); ++it, i++)
        {
            remoteIPinByte[i] = stoi(*it);
        }
        response->remoteIpAddr = remoteIPinByte[0] | remoteIPinByte[1] << 8 |
                                 remoteIPinByte[2] << 16 |
                                 remoteIPinByte[3] << 24;
