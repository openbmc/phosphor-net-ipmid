                                     ...
                                     const message::Handler& handler)
{
    ...
    if (!request->encryption && session->encrypted)
    {
    ...
    session->channelPtr = handler.channel;
    ...
    response->inPayloadSize = endian::to_ipmi<uint16_t>(sol::MAX_PAYLOAD_SIZE);
    response->outPayloadSize = endian::to_ipmi<uint16_t>(sol::MAX_PAYLOAD_SIZE);
                                       ...
                                       const message::Handler& handler)
{
    ...
    try
    {
        ...
        SOLActivating(request->payloadInstance, sessionID);
                                      ...
                                      const message::Handler& handler)
{
    ...
    auto status = std::get<sol::Manager&>(singletonPool).isPayloadActive(1);
