    ...
    std::string getRemoteAddress() const
    {
        if (sockAddrSize == sizeof(sockaddr_in))
        {
            unsigned int intaddr =
                ntohl(reinterpret_cast<const sockaddr_in*>(&remoteSockAddr)
                          ->sin_addr.s_addr);
            return boost::asio::ip::make_address_v4(intaddr).to_string();
        }
        if (sockAddrSize == sizeof(sockaddr_in6))
        {
            auto addr =
                boost::asio::ip::make_address_v6(reinterpret_cast<const char*>(
