...
TEST(ConfAlgo, AES_CBC_128)
{
    ...
    auto blockSize = cipher::conf::AlgoAES128::AESCBC128BlockSize;
    auto headersize = cipher::conf::AlgoAES128::AESCBC128ConfHeader;

    auto cipherExpectedLen = ((payloadLen/blockSize) + 1) * blockSize;
    cipherExpectedLen += headersize;

    // Validate the size of the Cipher
    ASSERT_EQ(true, (cipher.size() == cipherExpectedLen));
