...
void Table::registerCommand(CommandID inCommand, std::unique_ptr<Entry>&& entry)
{
    ...
    if (command)
    {
        lg2::debug("Already Registered: {CMD}", "CMD", inCommand.command);
        return;
    }
                           ...
                           std::shared_ptr<message::Handler> handler)
{
    ...
    if (iterator == commandTable.end())
    {
        ...
        {
            lg2::info(
                "Table: refuse to forward session-zero command: lun: {LUN}, netFn: {NETFN}, cmd: {CMD}",
                "LUN", command.lun(), "NETFN", command.netFn(), "CMD",
                command.cmd());
            return;
        }
                                  ...
                                  std::shared_ptr<message::Handler> handler)
{
    ...
    if (!sessionless && (handler->sessionID == session::sessionZero))
    {
        ...
        lg2::info(
            "Table: Insufficient privilege for command: lun: {LUN}, netFn: {NETFN}, cmd: {CMD}",
            "LUN", command.lun(), "NETFN", command.netFn(), "CMD",
            command.cmd());
        return errResponse;
    }
