...
void EventLoop::handlerRmcpPacket()
{
    try
    {
...
int EventLoop::startEventLoop()
{
    ...
    if (listensFdCount == 1)
    {
        if (sd_is_socket_inet(SD_LISTEN_FDS_START, AF_UNSPEC, SOCK_DGRAM, 1, 0))
        {
            udpSocket = std::make_shared<boost::asio::ip::udp::socket>(
                *io, boost::asio::ip::udp::v4(), SD_LISTEN_FDS_START);
        }
    }
    ...
    {
        udpSocket = std::make_shared<boost::asio::ip::udp::socket>(
            *io, boost::asio::ip::udp::endpoint(boost::asio::ip::udp::v4(),
                                                IPMI_STD_PORT));
        if (!udpSocket)
        {
