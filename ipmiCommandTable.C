...
void IpmiCommandTable::Register(IpmiCommandTableEntry* i_array, uint32_t l_numOfEntries) {
...
void IpmiCommandTable::ExecuteCommand(uint32_t i_commandID, IpmiMessageHandler& io_sessCtrl) {
  do {
    std::lock_guard<std::mutex> l_lock(iv_cmdTblMapMutex);
    ...
    if (l_itor == iv_cmdTblMap.end()) {
      std::cerr<<"E> IpmiCommandTable:: Command Not found: 0x"<<std::hex<<i_commandID<<std::endl;
      ...
      *(io_sessCtrl.iv_responsePayload) = IPMICC_CMD_UNKNOWN;
      ...
      std::cout<<"I> IpmiCommandTable:: Command found: 0x"<<std::hex<<i_commandID<<std::endl;
