...
std::unique_ptr<Message> MessageParser::unflatten(std::vector<uint8_t>&
        inPacket, SessionHeader& authType)
{
    BasicHeader_t* rmcpHeaderPtr = reinterpret_cast<BasicHeader_t*>
                                   (inPacket.data());
    ...
    // Read the Session Header and invoke the parser corresponding to the header type
...
std::vector<uint8_t> MessageParser::flatten(Message* outMessage,
...
std::unique_ptr<Message> Ipmi15Parser::unflatten(std::vector<uint8_t>& inPacket)
{
    ...
    message->payloadType = Message::PayloadType::IPMI;
...
std::vector<uint8_t> Ipmi15Parser::flatten(Message* outMessage)
{
    ...
    packet.reserve(MessageParser::MESSAGE_MAX_PACKET_LENGTH);
    ...
    header->base.format.formatType = static_cast<uint8_t>(SessionHeader::IPMI15);
    header->sessSeqNum = 0;
...
std::unique_ptr<Message> Ipmi20Parser::unflatten(std::vector<uint8_t>& inPacket)
{
    ...
    SessionHeader_t* header = reinterpret_cast<SessionHeader_t*>(inPacket.data());

    message->payloadType = static_cast<Message::PayloadType>
                           (header->payloadType & 0x3F);
    message->bmcSessionID = endian::from_ipmi<uint32_t>(header->sessId);
    message->sessionSeqNum = endian::from_ipmi<uint32_t>(header->sessSeqNum);
    message->isPacketEncrypted = ((header->payloadType & PAYLOAD_ENCRYPT_MASK) ?
                                  true : false);
    message->isPacketAuthenticated = ((header->payloadType & PAYLOAD_AUTH_MASK) ?
                                      true : false);
...
std::vector<uint8_t> Ipmi20Parser::flatten(Message* outMessage)
{
    ...
    header->base.rmcpSeqNum = RMCP_SEQ;
    header->base.classOfMsg = RMCP_MESSAGE_CLASS_IPMI;
    header->base.format.formatType = static_cast<uint8_t>(SessionHeader::IPMI20);
...
bool Ipmi20Parser::addSequenceNumber(std::vector<uint8_t>& packet)
{
    ...
    else
    {
        auto session = std::get<session::Manager&>(singletonPool).getSession(
                           ...
                           session::RetrieveOption::RETRIEVE_OPTION_RC_SESSION_ID);
        if (session)
        {
            auto seqNum = session->getSeqNumber().increment();
