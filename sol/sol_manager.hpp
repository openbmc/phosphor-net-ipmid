...
class Manager
{
    ...
    /*
     * SOL Payload Instance is the key for the map, the value is a tuple
     * containing SOL Context and BMC Session ID
     */
    using SOLPayloadMap = std::map<uint8_t,
            std::tuple<std::unique_ptr<Context>, session::SessionID>>;
     ...
     */
    Context& getSOLPayload(uint8_t payloadInstance);
