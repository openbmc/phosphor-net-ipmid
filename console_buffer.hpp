...
class ConsoleData
{
     ...
     *
     * This API would read the host console data based on the requested size,
     * if the buffer size is less than the requested, then the available data is
     * returned. The data is read from the beginning of the buffer.
     *
     * @param[in] size - requested number of bytes
     *
     * @return buffer containing host console data
     */
    auto readConsoleData(size_t size)
    {
     ...
     */
    void writeConsoleData(buffer input)
    {
     ...
     *
     * @note If the console buffer has less bytes that that was requested, then
     *       the available size is erased.
     */
    void eraseConsoleBuffer(size_t size)
    {
