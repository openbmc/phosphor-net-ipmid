...
#define IPMI_CRYPT_AES_CBC_128_BLOCK_SIZE 0x10
  ...
  if (1) {
    ...
    if (mod) pad_length = IPMI_CRYPT_AES_CBC_128_BLOCK_SIZE - mod;

    padded_input = (uint8_t*)malloc(i_PayloadLen + pad_length + 1);
    if (padded_input == nullptr) return;
...
void IpmiConfidentialityAlgoAesCbc128::decryptData(IpmiSessionKeys* i_sessionSlot,
  ...
  int x =ipmi_decrypt_aes_cbc_128(input,                                       /* IV              */
    ...
    if (x == -1) {
      std::cerr<<"E> Error in encryption"<<std::endl;
      delete[] output;
      return ;
    }
...
bool IpmiStaticPasswordAuthentication::AuthenticateUser(uint8_t* i_userName, uint32_t i_userNameLen,
