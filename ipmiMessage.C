...
//Outgoing Message
IpmiMessage::IpmiMessage(PacketField32_t i_sessionId, PacketField32_t i_bmcSessionId,
    ...
    : iv_isFragment(true),
      ...
      iv_integrityData(nullptr),
      iv_integrityDataRequiresDelete(false) {
  memcpy(iv_payload,i_pMessage,iv_payloadLength.get());

  if (i_payloadType != IPMI_PAYLOAD_TYPE_IPMI) {
    iv_parser = new Ipmi20Parser;
  } else if (i_inMsg) {
    iv_parser = IpmiMessageParser::getParser(i_inMsg->getPacket(),i_inMsg->getPacketLength());
  } else {
...
void IpmiMessage::reset() {
  ...
  iv_isPacketAuthenticated=false;
  iv_payloadType=IPMI_PAYLOAD_TYPE_INVALID;
  iv_sessionId.set(IPMI_MESSAGE_INVALID_SESSION_ID);
  iv_sessionSeqNum.set(0);
  iv_packetLength=0;
...
//Get Payload Type
uint8_t IpmiMessage::getPayloadType(void) { return iv_payloadType; }
...
PacketField32_t& IpmiMessage::getSessionId(void) {
  return iv_sessionId;
}

PacketField32_t& IpmiMessage::getBmcSessionId(void) {
  return iv_bmcSessionId;
}

void IpmiMessage::setSessionId(PacketField32_t& i_sessionID) {
  iv_sessionId.set(i_sessionID.get());
}

void IpmiMessage::setBmcSessionId(PacketField32_t& i_sessionID) {
  iv_bmcSessionId.set(i_sessionID.get());
}

//Get the session Sequence Number - Host-ordered
PacketField32_t& IpmiMessage::getSessionSeqNum(void) {
  return iv_sessionSeqNum;
}

void IpmiMessage::setSessionSeqNum(PacketField32_t& i_val) {
  iv_sessionSeqNum.set(i_val.get());
}

bool IpmiMessage::getIsPacketEncrypted() { return iv_isPacketEncrypted; }
...
bool IpmiMessage::getIsPacketAuthenticated() { return iv_isPacketAuthenticated; }
...
//Get the packet
uint8_t* IpmiMessage::getPacket(void) { return iv_packet; }
...
uint8_t* IpmiMessage::getPayload(void) { return iv_payload; }
...
PacketField16_t& IpmiMessage::getPayloadLength(void) { return iv_payloadLength; }

void IpmiMessage::setPayloadLength(PacketField16_t& i_val) {
  iv_payloadLength.set(i_val.get());
}

uint8_t* IpmiMessage::getIntegrityData(void) { return iv_integrityData; }
...
uint32_t IpmiMessage::getIntegrityDataLength(void) { return iv_integrityDataLength; }
...
int IpmiMessage::Receive(IpmiSockChannelData &i_channel) {
  ...
  do {
    ...
    if(iv_parser == nullptr) {
      ...
      if (l_channelStatus < 0) {
        std::cerr<<"E> Error in Read (after failed verify) :"<<std::hex<<l_rc<<std::endl;
      }
      l_rc = -1;
      iv_packet = nullptr;
      break;
    ...
    std::cout<<"Read>> Got parser: read packet length"<<iv_packetLength<<std::endl;
...
void IpmiMessage::logBuffer(uint8_t* i_buffer, uint32_t i_bufferLen, bool l_outMsg,
                            uint16_t i_remotePort ) {
    char logbuffer[(3*i_bufferLen) + 26];
    int index = 0;
    if(l_outMsg)
    {
        sprintf(logbuffer,"0x%8X : Tx: %5d ",iv_bmcSessionId.get(), i_remotePort);
    }
    else
    {
        sprintf(logbuffer,"0x%8X : Rx: %5d ",iv_sessionId.get(), i_remotePort);
    }
    index += 23;
...
void IpmiMessage::unflatten() {
  if(iv_parser && iv_parser->unflatten(this)) {
    ...
    std::cerr<<"E> IpmiMessage::unflatten : Error no parser available"<<std::endl;
  }
}
